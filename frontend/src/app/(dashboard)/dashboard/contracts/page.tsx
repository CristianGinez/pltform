...
const CONTRACT_STATUS_CONFIG: Record<ContractStatus, { label: string; pill: string }> = {
  ACTIVE:     { label: 'Activo',      pill: 'bg-blue-50 text-blue-700' },
  COMPLETED:  { label: 'Completado',  pill: 'bg-emerald-50 text-emerald-700' },
  DISPUTED:   { label: 'En disputa',  pill: 'bg-orange-50 text-orange-700' },
  CANCELLED:  { label: 'Cancelado',   pill: 'bg-gray-100 text-gray-500' },
};

const GROUP_ORDER: ContractStatus[] = ['ACTIVE', 'DISPUTED', 'COMPLETED', 'CANCELLED'];

const GROUP_HEADERS: Record<ContractStatus, { label: string; icon: React.ReactNode; color: string }> = {
  ACTIVE:    { label: 'Activos',       icon: <Clock size={13} />,          color: 'text-blue-600' },
  DISPUTED:  { label: 'En disputa',    icon: <AlertTriangle size={13} />,  color: 'text-orange-500' },
  COMPLETED: { label: 'Completados',   icon: <CheckCircle size={13} />,    color: 'text-emerald-600' },
  CANCELLED: { label: 'Cancelados',    icon: <XCircle size={13} />,        color: 'text-gray-400' },
};
...
function CompanyCard({ project }: { project: ProjectWithContract }) {
  const cfg = CONTRACT_STATUS_CONFIG[project.contract?.status ?? 'ACTIVE'];
    ...
    <Link href={href} className="block group">
      <div className="bg-white rounded-xl border border-gray-100 p-5 hover:border-gray-200 hover:shadow-sm transition-all">
        <div className="flex items-start justify-between gap-3 mb-3">
          ...
          <span className={`text-xs px-2.5 py-1 rounded-full font-medium shrink-0 ${cfg.pill}`}>{cfg.label}</span>
        </div>
...
function DevCard({ proposal }: { proposal: Proposal }) {
  ...
  const contractStatus = project?.contract?.status ?? 'ACTIVE';
  const cfg = CONTRACT_STATUS_CONFIG[contractStatus];
  const href = project?.contract ? `/dashboard/contracts/${project.contract.id}` : '#';
...
function Section({ status, children, count }: { status: ContractStatus; children: React.ReactNode; count: number }) {
  const h = GROUP_HEADERS[status];
  return (
    <div>
      <div className={`flex items-center gap-1.5 mb-3 ${h.color}`}>
        {h.icon}
        <span className="text-xs font-semibold uppercase tracking-wide">{h.label}</span>
...
export default function ContractsPage() {
  ...
  // Company: all projects that have a contract
  const projectsWithContract = (allProjects as ProjectWithContract[]).filter((p) => !!p.contract);

  // Developer: all accepted proposals
  const acceptedProposals = allProposals.filter((p) => p.status === 'ACCEPTED');

  // Group by contract status
  const companyGroups = GROUP_ORDER.reduce<Record<ContractStatus, ProjectWithContract[]>>(
    (acc, s) => ({ ...acc, [s]: projectsWithContract.filter((p) => (p.contract?.status ?? 'ACTIVE') === s) }),
    {} as Record<ContractStatus, ProjectWithContract[]>,
  );

  const devGroups = GROUP_ORDER.reduce<Record<ContractStatus, Proposal[]>>(
    (acc, s) => ({
      ...acc,
      [s]: acceptedProposals.filter((p) => {
        const proj = p.project as ProjectWithContract | undefined;
        return (proj?.contract?.status ?? 'ACTIVE') === s;
      }),
    }),
    {} as Record<ContractStatus, Proposal[]>,
  );
      ...
      {!isLoading && hasAny && (
        <div className="space-y-8">
          {GROUP_ORDER.map((status) => {
            const items = isCompany ? companyGroups[status] : devGroups[status];
            if (!items.length) return null;
            return (
              <Section key={status} status={status} count={items.length}>
                {isCompany
                  ? (items as ProjectWithContract[]).map((p) => <CompanyCard key={p.id} project={p} />)
                  : (items as Proposal[]).map((p) => <DevCard key={p.id} proposal={p} />)}
              </Section>
            );
          })}
        </div>
